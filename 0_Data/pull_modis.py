...
CLEAN_NAME = (
    lambda r, l: unidecode(r.get("properties").get(l))
    .lower()
...
FTR_KEY_FNS = [
    lambda region: CLEAN_NAME(region, "partido")
    + "-"
    + CLEAN_NAME(region, "provincia"),  # ARG: "<county name>-<province name>"
    lambda region: CLEAN_NAME(region, "NM_MESO")
    + "-brasil",  # BR: "<mesoregion name>-brasil"
    lambda region: CLEAN_NAME(region, "DISTRICT")
    + "-"
    + CLEAN_NAME(region, "ST_NM"),  # IN: "<district name>-<state name>"
    lambda region: CLEAN_NAME(region, "NAME")
    + "-"
    + GET_FIPS(region, "STATEFP"),  # US: "<county name>-<state name>"
    lambda region: CLEAN_NAME(region, "ADMIN2")
    + "-"
    + CLEAN_NAME(region, "ADMIN1"),  # ET: "<zone name>-<state name>"
]
...
FTR_FILTER_FNS = [
    ...
    lambda region: True,
    lambda region: True,
    lambda region: region.get("properties").get("STATEFP")
    in USA_SOY_FIPS_CODES,  # US: only pull imagery from states with soy production
    lambda region: True,
]


USAGE_MESSAGE = (
    "Usage: python pull_modis.py <"
    + ", ".join(IMG_COLLECTION_CODES)
    + "> <"
    + ", ".join(REGIONS)
    + ">"
)  # + '<folder in bucket/ (optional)>'
NUM_ARGS = 3
NUM_OPTIONAL_ARGS = 1
...
def shapefile_to_ee_feature_collection(
    shapefile_path, province_name, region_column_name
):
    # Read the shapefile
    gdf = gpd.read_file(shapefile_path)
    gdf = gdf[
        gdf[region_column_name].apply(lambda x: x.replace(" ", "")) == province_name
    ]
    ...
    for _, row in gdf.iterrows():
        ...
        if geom_type == "Polygon":
            ...
            geom = ee.Geometry.Polygon(
                [list(x) for x in row["geometry"].exterior.coords[:]]
            )
...
def export_to_drive_GEE(
    img, fname, folder, expregion, scale, eeuser=None
):  # This exports to Google Drive.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Pull MODIS data for specified countries and imagery types."
    )
    parser.add_argument(
    ...
    for collection_name in IMG_COLLECTION_CODES:
        ...
        if use_gee_shapefile:
            ...
            all_regions = [
                ...
                for feature in feature_list_computed
                if "properties" in feature and "partido" in feature["properties"]
            ]
            if specified_regions[0] != "all":
                feature_list_computed = [
                    feature
                    for feature in feature_list_computed
                    if feature["properties"]["partido"].lower() in specified_regions
                ]
        else:
            if len(specified_regions) > 1:
                ...
                exit(1)
            feature_list_computed = shapefile_to_ee_feature_collection(
                shapefile_path, specified_regions[0], region_column_name
            )
            if len(feature_list_computed.getInfo()["features"]) > 1:
                print(":: Error: Something is wrong reading the local shapefile.")
                print(":: Download stopped.")
                exit(1)
            feature_list_computed = ee.Feature(
                feature_list_computed.getInfo()["features"][0]
            )
        ...
        if (
            img_collection_index != 1
        ):  # temperature index <<< is this min max filtering needed?
        ...
        if use_gee_shapefile:
            for idx, region in enumerate(feature_list_computed):
                ...
                if drive_folder is not None and os.path.isfile(
                    os.path.join(BUCKET_VM_REL, drive_folder, file_name + ".tif")
                ):
                ...
                try:
                    export_to_gcs_bucket(
                        img, file_name, drive_folder, ee.Feature(region), scale
                    )
            ...
            file_name = (
                region_name + "_" + collection_name + "_" + start_date + "_" + end_date
            )
            if drive_folder is not None and os.path.isfile(
                os.path.join(BUCKET_VM_REL, drive_folder, file_name + ".tif")
            ):
            ...
            try:
                export_to_gcs_bucket(
                    img, file_name, drive_folder, feature_list_computed, scale
                )
            except KeyboardInterrupt:
                print("received SIGINT, terminating execution")
                break
        ...
        if use_gee_shapefile:
            print(
                ...
                len(feature_list_computed)
                - len(keys_with_issues)
                - count_already_downloaded
                - count_filtered,
                "new tifs from GEE",
            )
        else:
            print(
                "Successfully ordered",
                1 - len(keys_with_issues) - count_already_downloaded - count_filtered,
                "new tifs.",
            )
