...
from os.path import basename, expanduser, getsize, isdir, isfile, join
...
def return_files(path):
    return [
        f for f in listdir(path) if (isfile(join(path, f)) and not f.startswith("."))
    ]
...
def analyze_histograms(directory, save_directory):
    ...
    for idx, f in enumerate(hist_files):
        ...
        if idx == 0:
            ...
            mode_matrix = np.empty(
                (num_bands, len(SUMMARY_STAT_NAMES), len(hist_files))
            )
...
def return_tif_filenames(path):
    return [
        ...
        if (isfile(join(path, f)) and not f.startswith(".") and f.endswith(".tif"))
    ]
...
def mask_image(
    ...
    mask,
    num_bands,
    num_years_extend_backward,
    num_years_extend_forward,
    num_imgs_per_year,
):
    """
    ...
    for t in range(num_imgs):
        ...
        if mask_year < num_years_extend_backward:
            ...
            assert (
                mask_year
                < mask.shape[2] + num_years_extend_backward + num_years_extend_forward
            )
            mask_slice = mask[:, :, -1]
        else:
            mask_slice = mask[:, :, mask_year - num_years_extend_backward]
        for b in range(num_bands):
            img[:, :, t * num_bands + b] = np.multiply(
                img[:, :, t * num_bands + b], mask_slice
            )
    return img
...
def collect_tif_path_dict(sat_dir, temp_dir, mask_dir, verbose=True):
    ...
    for s_i, place in enumerate(sat_places):
        if place not in temp_places or place not in mask_places:
            if verbose:
                print(place, "missing temp and/or mask file")
            continue
...
if __name__ == "__main__":
    ...
    for place, tif_path_tuple in tif_dict.items():
        ...
        dates_raw = sat_tensor[
            int(sat_tensor.shape[0] / 2),
            int(sat_tensor.shape[0] / 2),
            indices_to_remove,
        ]
        if "mali" in sat_path:
            dates_raw[604] = (
                56  # Manual correction because of MODIS error in Mali. Only valid if start_date = 2003-01-01.
            )
        ...
        if (
            sat_tensor.shape[:2] != temp_tensor.shape[:2]
            or sat_tensor.shape[:2] != mask_tensor.shape[:2]
        ):
            print(
                place,
                "slice shapes do not match! sat, temp, mask shapes:",
                sat_tensor.shape[:2],
                temp_tensor.shape[:2],
                mask_tensor.shape[:2],
            )
        ...
        mask_years_missing = (
            int(num_temp_imgs / num_imgs_per_year)
            - NUM_YEARS_EXTEND_FORWARD
            - mask_tensor.shape[2]
        )
        # assert mask_years_missing == 0
        sat_tensor = mask_image(
            sat_tensor,
            mask_tensor,
            8,
            mask_years_missing,
        ...
        if not indices_only:
            ...
            temp_histograms = calc_histograms(
                temp_tensor,
                HIST_BINS_LIST[NUM_REF_BANDS : NUM_REF_BANDS + NUM_TEMP_BANDS],
            )
