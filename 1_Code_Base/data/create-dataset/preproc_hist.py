...
def try_numpy_load(file_path):
    try:
        return np.load(file_path)
    except Exception as e:
        return np.zeros(60 * 9)
...
for file in os.listdir(test_path):
    ...
    for year in os.listdir(yearly_data_dir):
        ...
        if len(histogram_files) >= 2:
            combined_data = [
                try_numpy_load(os.path.join(monthly_data_dir, f"{i}_{i+1}.npy"))
                for i in range(5, 10, 2)
            ]
            df = pd.DataFrame(np.vstack(combined_data))
            df["year"] = year
            df["state_fips"] = state
            df["county_fips"] = county_fips
            np.save(
                os.path.join(
                    output_path, f"histogram-{state}-{county}-{year}-{county_fips}.npy"
                ),
                df.to_numpy(),
            )
