...
class LstmModel(keras.Model):
    def __init__(self, input_shape, lstm_layers=3, no_units=2, output_units=1, dropout_rate=0.2):
        ...
        for i in range(lstm_layers):
            if i == 0:
                self.lstm_layers.append(LSTM(no_units, return_sequences=(lstm_layers > 1)))
            else:
                self.lstm_layers.append(LSTM(no_units, return_sequences=(i < lstm_layers - 1)))
...
# Main training loop
def train_and_evaluate(model, train_dataset, val_dataset, epochs=10000, learning_rate=0.001, patience=10):
...
if __name__ == "__main__":
  ...
  # Load data
    train_data = load_data(os.path.join(dataset_source_dir, 'train_hists.npz'), dtype=np.float32)
    train_labels = load_data(os.path.join(dataset_source_dir, 'train_yields.npz'), dtype=np.float32)
    dev_data = load_data(os.path.join(dataset_source_dir, 'dev_hists.npz'), dtype=np.float32)
    dev_labels = load_data(os.path.join(dataset_source_dir, 'dev_yields.npz'), dtype=np.float32)
    test_data = load_data(os.path.join(dataset_source_dir, 'test_hists.npz'), dtype=np.float32)
    test_labels = load_data(os.path.join(dataset_source_dir, 'test_yields.npz'), dtype=np.float32)
